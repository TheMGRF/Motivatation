package me.themgrf.motivatation.util;

public class Colour {

    public static final String WHITE = "#FFFFFF";
    public static final String GREY = "#CCCCCC";
    public static final String DARK_GREY = "#808080";
    public static final String BLUE = "#0066FF";
    public static final String PURPLE = "#9933FF";
    public static final String ORANGE = "#FF9966";
    public static final String GOLD = "#FFD700";
    public static final String CYAN = "#00FFFF";
    public static final String GREEN = "#00FF00";
    public static final String DARK_GREEN = "#33CC33";
    public static final String YELLOW = "#FFFF00";
    public static final String AMBER = "#FFCC00";
    public static final String LIGHT_RED = "#FF4D4D";
    public static final String RED = "#FF0000";
    public static final String DARK_RED = "#800000";

}
